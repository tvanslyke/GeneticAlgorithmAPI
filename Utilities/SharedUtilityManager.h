...
protected:
	 ...
	 */
	std::function<void (ValueType * v)> makeDeleter(const KeyType & key)
	{
		return  [this, key](ValueType * v){  this->utilities_.erase(key);  delete v; };
	}
public:
	/**
	 ...
	 */
	std::shared_ptr<ValueType> get(KeyType key)
	{
...
protected:
	 ...
	 * will also hold this deleter.
	 */
	std::function<void (ValueType * v)> makeDeleter(const GenericWeakKey & key)
	{
		return  [this, key](ValueType * v){  this->utilities_.erase(key);  delete v; };
	}
public:
	 ...
	 * public method.
	 */
	bool has(const GenericWeakKey & key) noexcept
	{
		return utilities_.count(key) > 0;
	}
